...
class BoardQueryDaoTest {
    ...
    class Describe_searchBoard {
        ...
        class Context_with_20_data_and_a_Pageable_with_size_10_and_the_1st_page {
            ...
            // 페이지는 0번째 부터 시작한다.
            @DisplayName("1번째의 페이지인 10개의 데이터가 반환된다.")
            @Test
            void it_returns_BoardDto_List_of_size_10() {
                List<BoardDto> boardDtos = boardQueryDao.searchBoards(pageable);

                assertThat(boardDtos).hasSize(10);
                // 생성된 기준으로 정렬 확인
                assertThat(boardDtos).element(0)
                        .extracting("title")
                        .isEqualTo("title9");
            }
        }
        ...
        class Context_with_20_data_and_a_Pageable_with_size_20_and_the_0st_page {
            ...
            @DisplayName("0번째의 페이지인 20개의 데이터가 반환된다.")
            @Test
            void it_returns_BoardDto_List_of_size_20() {
                List<BoardDto> boardDtos = boardQueryDao.searchBoards(pageable);

                assertThat(boardDtos).hasSize(20);
                // 생성된 기준으로 정렬 확인
                assertThat(boardDtos).element(0)
                        .extracting("title")
                        .isEqualTo("title19");
